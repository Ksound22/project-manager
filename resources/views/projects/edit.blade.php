<x-app-layout>
  <h1 class="text-3xl text-gray-100 font-bold text-center mt-3">Edit Project: {{ $project->name }}</h1>
    ...
    <form action="{{ route('projects.update', $project) }}" method="POST">
      ...
      <div class="mt-4">
        ...
        <x-select id="status" name="status" 
            :options="['pending' => 'Pending', 'in-progress' => 'In Progress', 'completed' => 'Completed', 'on-hold' => 'On Hold']" 
      ...
      <div class="mt-4">
        ...
        <x-select id="priority" name="priority" :options="['low' => 'Low', 'medium' => 'Medium', 'high' => 'High']" selected="medium" :selected="old('priority', $project->priority)" /> />
        <x-input-error :messages="$errors->get('priority')" class="mt-2" />
      </div>

      <div class="mt-4">
        <x-input-label for="date" :value="__('Date')" />
        <x-date-input id="date" name="deadline" value="{{ old('deadline', $project->deadline) }}" />
